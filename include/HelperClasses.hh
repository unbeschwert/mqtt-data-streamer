...
public:
  ...
  TopicsToHandle(const std::string &name_, uint8_t QoS_)
      : name(name_), QoS(QoS_) {}
...
class ActionListener : public virtual mqtt::iaction_listener {
  ...
  void on_failure(const mqtt::token &tok) override {
    ...
    if (topics && !topics->empty())
      std::cout << "\t" << name << " failure for " << (*topics)[0] << '\n';
    done = true;
  }
  void on_success(const mqtt::token &tok) override {
    auto topics = tok.get_topics();
    if (topics && !topics->empty())
      std::cout << "\t" << name << " success for " << (*topics)[0] << '\n';
    done = true;
  }

public:
  ActionListener(const std::string &name_) : name(name_), done(false) {}
...
class MqttCallback : public virtual mqtt::callback {
  ...
  void connected(const std::string &cause) override {
    ...
    for (const auto &topic : topics_to_handle) {
      std::cout << "\t\tSubscribing to '" << topic->name << "' using QoS '"
                << topic->QoS << "'\n";
      mqtt_async_client->subscribe(topic->name, topic->QoS, nullptr, listener);
    }
  ...
  void message_arrived(mqtt::const_message_ptr msg) override {
    std::cout << "\tMessage arrived on " << msg->get_topic() << "\n";
    for (const auto &topic : topics_to_handle) {
...
public:
  MqttCallback(
      std::shared_ptr<mqtt::async_client> mqtt_async_client_,
