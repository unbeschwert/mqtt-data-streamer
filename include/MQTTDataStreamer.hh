 ...
 */
using MqttAsyncTuple = std::tuple<mqtt::async_client_ptr, mqtt::callback_ptr>;
...
class MQTTDataStreamer {
   ...
   */
  mqtt::message_ptr createMessage(const void *payload, std::size_t len,
...
public:
   ...
   */
  void publishMessage(const void *payload, std::size_t len, std::string topic,
                      uint8_t QoS, std::mutex *mut,
