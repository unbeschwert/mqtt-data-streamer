...
MQTTDataStreamer::MQTTDataStreamer(MqttAsyncTuple mqtt_async_tuple_,
...
mqtt::connect_options MQTTDataStreamer::buildConnectOptions() {
  ...
  opt.set_will(
      mqtt::message("/demonstrator_04/distress", lwt_payload, 1, false));
  return (opt);
}

mqtt::message_ptr MQTTDataStreamer::createMessage(const void *payload,
                                                  std::size_t len,
  ...
  mqtt::message_ptr msg =
      mqtt::make_message(topic, payload, len, QoS, retain_msg);
  std::cout << "Finished!\n";
  return (msg);
}

void MQTTDataStreamer::publishMessage(const void *payload, std::size_t len,
                                      std::string topic, uint8_t QoS,
                                      std::mutex *mut,
  ...
  try {
    ...
    mqtt::delivery_token_ptr publish_tok =
        std::get<0>(mqtt_async_tuple)->publish(msg, nullptr, listener);
